...
#ifdef MINGWSTD
    thread(const thread& __t) = delete;

    thread(thread&& __t)
    :mHandle(__t.mHandle), _M_id(__t._M_id)
    {
      __t.mHandle = kInvalidHandle;
      __t._M_id = id{};
    }
...
moving another thread to it.\n");
    ...
    {
        std::swap(mHandle, __t.mHandle);
        std::swap(_M_id._M_thread, __t._M_id._M_thread);
    }
